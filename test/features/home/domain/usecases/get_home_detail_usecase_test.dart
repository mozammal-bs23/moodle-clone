...
import 'package:flutter_boilerplate/features/home/domain/usecases/get_home_detail_usecase.dart';
...
void main() {
  ...
  group('GetHomeDetailUseCase', () {
    ...
    test('calls repository.getHomeDetail with correct id', () async {
      // Arrange
      when(mockRepository.getHomeDetail(testId))
    ...
    test('propagates correct id parameter', () async {
      // Arrange
      when(mockRepository.getHomeDetail('id-1'))
          .thenAnswer((_) async => (tHomeEntity, null));
      when(mockRepository.getHomeDetail('id-2'))
          .thenAnswer((_) async => (tHomeEntity, null));
